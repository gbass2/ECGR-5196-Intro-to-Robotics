...
void driveStraight(uint32_t distance, bool direction, uint8_t wheelSpeed) {
  ...
  resetLeftEncoderCnt();  resetRightEncoderCnt();  // Set encoder pulse
                                                  //count back to 0
  setMotorDirection(BOTH_MOTORS,direction);  // Cause the robot to drive in the
                                            // direction specified
  ...
  while(leftTotalCount<(straight-50) || rightTotalCount<(straight-50)) {
    ...
    if(leftTotalCount + 1 < rightTotalCount){
      // If the speed of the left wheel speed is less than the defined
      // threashold, then increase the left wheel speed.
      // If it is greater than the defined threashold, then reset
      // the right wheel speed to the default.
      if(wheelSpeedL < 20) {
       wheelSpeedL++;
      } else {
    ...
    if(rightTotalCount + 1 < leftTotalCount){
      // If the speed of the right wheel speed is less than the defined
      // threashold, then increase the right wheel speed.
      // If it is greater than the defined threashold, then reset the
      // left wheel speed to the default.
      if(wheelSpeedR < 20) {
       wheelSpeedR++;
      } else {
...
void turnInPlace(uint16_t degrees, bool direction) {
   ...
   if(direction == LEFT){
     ...
     wheelSpeedL = 21;
     // Set up the motors
     setMotorDirection(RIGHT_MOTOR,MOTOR_DIR_FORWARD);  // Set the right motor
                                                        // to go forwards
     setMotorDirection(LEFT_MOTOR,MOTOR_DIR_BACKWARD);  // Set the left motor
                                                        // to go backwards
     ...
     while(getEncoderRightCnt()<totalEncoderCount &&
     getEncoderLeftCnt()<totalEncoderCount);       // stay in loop
  } else {
     wheelSpeedR = 20;
     wheelSpeedL = 21;
     // Turn in place right
